...
namespace GoogleSheetsToUnity
{
    ...
    {      
        ...
        static IEnumerator Read(string url, string titleColumn, int titleRow, OnSpreedSheetLoaded callback)
        {
            ...
            {
                DebugPro.LogError($"Failed to load spreadsheet: {request.error}");
                yield break;
            }
            ...
            try
            {
                rawData = JsonConvert.DeserializeObject<ValueRange>(request.downloadHandler.text);
            }
            catch (System.Exception ex)
            {
                DebugPro.LogError($"JSON parse error: {ex.Message}");
                yield break;
            }
            
            var response = new GSTU_SpreadsheetResponce(rawData);
            var spreadsheet = new GstuSpreadSheet(response, titleColumn, titleRow);
