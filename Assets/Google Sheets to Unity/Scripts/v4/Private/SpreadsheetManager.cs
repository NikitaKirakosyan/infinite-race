    ...
    {   
        ...
        static IEnumerator Read(UnityWebRequest request, GSTU_Search search, bool containsMergedCells, UnityAction<GstuSpreadSheet> callback)
        {
            ...
            using (request)
            {
                ...
                var rawData = JsonConvert.DeserializeObject<ValueRange>(request.downloadHandler.text);
                var responce = new GSTU_SpreadsheetResponce(rawData);
                ...
                if (containsMergedCells)
                {
                    ...
                    var request2 = UnityWebRequest.Get(sb.ToString());

                    yield return request2.SendWebRequest();

                    var root = JsonConvert.DeserializeObject<SheetsRootObject>(request2.downloadHandler.text);
        ...
        public static void Write(GSTU_Search search, ValueRange inputData, UnityAction callback)
        {
            ...
            var request = UnityWebRequest.Put(sb.ToString(), bodyRaw);
