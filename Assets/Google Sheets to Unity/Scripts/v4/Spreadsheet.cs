...
namespace GoogleSheetsToUnity
{
    ...
    public class GSTU_Search
    {
        ...
        public readonly string endCell = "Z1000";
    ...
    public class GSTU_SpreadsheetResponce
    {
        ...
        public string StartCell()
        {
            ...
            if(end == -1)
                return valueRange.range.Substring(start);
        ...
        {
            var colon = valueRange.range.IndexOf(":");
            if(colon == -1)
                return valueRange.range.Substring(valueRange.range.IndexOf("!") + 1);
            return valueRange.range.Substring(colon + 1);
        }
   ...
   /*     public GstuSpreadSheet(GSTU_SpreadsheetResponce data)
        ...
        public GstuSpreadSheet(GSTU_SpreadsheetResponce data, string titleColumn, int titleRow)
        {
            ...
            foreach (var dataValue in data.valueRange.values)
            {
        ...
        public GSTU_Cell this[string rowId, string columnId]
        {
            get
            {
                    var columnIndex = columns.secondaryKeyLink[columnId];
                    var rowIndex = rows.secondaryKeyLink[rowId];
        ...
        public List<GSTU_Cell> this [string rowID, string columnID, bool mergedCells]
        {
            get
            {
                var columnIndex = columns.secondaryKeyLink[columnID];
                var rowIndex = rows.secondaryKeyLink[rowID];
                var actualCells = Cells[columnIndex + rowIndex].titleConnectedCells;

                var returnCells = new List<GSTU_Cell>();
                foreach(var s in actualCells)
                {
